...
function App() {
  ...
  return <main className="shell">
    ...
    {candidates.length > 0 && <section><h2>候选方案</h2><div className="grid">{candidates.map((candidate) => <article className="candidate" key={candidate.candidate_id}><div className="candidate-head"><h3>{candidate.candidate_id}</h3><span>{candidate.scores?.structural_validity ?? "—"}</span></div>{(candidate.artifacts?.png || candidate.artifacts?.svg) && <img src={candidate.artifacts.png || candidate.artifacts.svg} alt="候选预览" onError={(event) => { event.currentTarget.style.display = "none"; }} />}<p className="muted">结构、版式和视觉层级候选均保留原始 Figure Spec。</p></article>)}</div></section>}
  </main>;
}
