...
它还没有达到“自动生成可直接投稿的最终科研图”阶段。主要原因是论文语义解析仍以规则和线性流程为主，视觉精修尚未形成完整的自动迭代闭环，网络模板检索目前是本地可审计目录，Figma 写入已经验证但还没有把 MCP 调用封装为一个自动化批处理命令。

## 已识别风险

- 复杂论文中的循环、分支、跨层依赖可能被简化为线性边；系统会保留证据，但仍需要语义审阅。
